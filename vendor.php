...
<div class="header">
		...
		<div class="container">
			...
			?>
			<div class="header">
				<b>Welcome </b><b style="color: green;"><?php echo 'customer' ?></b>
			</div>
				...
				<div class="form-w3agile">
					...
					<?php
						if(isset($_POST['submit']))
						{
							...
							if($email != "" && $passwd != ""){
								$query= "SELECT * from vendors where email='".$email."' AND password='".$passwd."'";
								$data= mysqli_query($conn, $query);
								$total= mysqli_num_rows($data);
								if($total == 1){ 
									// returns the row of data of from the query abouve 
									$result = $data->fetch_row();
									if($result[10] == 1){
										$_SESSION['user']= $email;
										echo "<script type='text/javascript'>  window.location='vendor-index.php'; </script>";
									}else{
										echo "The vendor profile has not been validated by the admin.";
									}	
								}
								else{
							...
							if ($result = mysqli_query($conn, "SELECT * FROM Persons")) {
