    ...
    <?php include "./templates/sidebar.php"; ?>
      ...
      <div class="row">
        ...
        <?php 
          ...
          order_summary.order_id = orders.order_id join products on order_summary.product_id = products.product_id join customers on orders.customer_id = customers.id WHERE 
          orders.pre_order=0 and products.vendor_name = '$uname' order by orders.created_at DESC");
          $total= 0;
          if ($query->num_rows > 0) {                
          ...
          if ($query->num_rows > 0) { 
            $i=0; foreach($result as $row){ $i++; ?> 
              <tr>
                ...
                <a href="#" data-toggle="modal" data-target="#change_delivery" class="btn btn-info btn-md"  onclick="change_delivery_status('<?= $row[46] ?>' ,<?= $row[0] ?>)">
                Change Delivery Status</a></td>
              </tr>
          <?php } }?>
          </tbody>
        </table>
